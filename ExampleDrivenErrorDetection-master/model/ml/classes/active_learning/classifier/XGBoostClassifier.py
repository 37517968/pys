...
class XGBoostClassifier(object):
    ...
    def __init__(self, X_train, X_test, balance=False):
        ...
        # 保存原始数据用于重建DMatrix
        self._X_train_data = X_train
        self._X_test_data = X_test
    
    def __getstate__(self):
        """自定义pickle序列化方法，避免序列化DMatrix对象"""
        state = self.__dict__.copy()
        # 移除不能被pickle的DMatrix对象
        state.pop('X_train', None)
        state.pop('X_test', None)
        state.pop('x_alll', None)
        return state
    
    def __setstate__(self, state):
        """自定义pickle反序列化方法，重建DMatrix对象"""
        self.__dict__.update(state)
        # 重建DMatrix对象
        if hasattr(self, '_X_train_data'):
            self.X_train = xgb.DMatrix(self._X_train_data)
        if hasattr(self, '_X_test_data'):
            self.X_test = xgb.DMatrix(self._X_test_data)
        self.x_alll = None
    ...
    def train_predict_all(self, x, y, column_id, x_all, x_all_test):
        ...
        if type(None) != type(x_all_test):
            # 确保测试数据的特征数量与训练数据相同
            if x_all_test.shape[1] != x.shape[1]:
                # 如果特征数量不同，调整测试数据的特征数量
                if x_all_test.shape[1] > x.shape[1]:
                    # 截断多余的特征
                    x_all_test = x_all_test[:, :x.shape[1]]
                else:
                    # 填充缺失的特征
                    import numpy as np
                    from scipy.sparse import hstack, csr_matrix
                    padding = csr_matrix((x_all_test.shape[0], x.shape[1] - x_all_test.shape[1]))
                    x_all_test = hstack((x_all_test, padding))
