...
def create_user_start_data(feature_matrix, target, num_errors=2, return_ids=False):
    ...
    # 与错误数据相同数量的正确数据
    # correct_select_ids = correct_select_ids[0:min(num_errors, len(correct_select_ids))]
    # 保留所有正确数据
    correct_select_ids = correct_select_ids[0:len(correct_select_ids)]
...
def create_train_data(dataSet, current_predictions_test, num_samples=5):
    """
    作用：
        1. 计算测试数据集中每一行（样本）的平均预测置信度；
        2. 选出置信度最低的5条样本；
        3. 将这5条样本交给 LLM 生成更多相似训练数据；
        4. 将生成的数据合并到 dataSet 中，更新相关参数。

    参数：
        dataSet : DataSet 对象（例如 HospitalHoloCleanLLMGen 或其他）
        current_predictions_test : dict[column_id -> per-cell predictions（概率或标签）]
                                   通常是模型在测试集上的输出结果
    ...
    # 如果 current_predictions_test 是一个字典（每列一个数组），合并成矩阵
    # 假设每个列预测都是概率或0/1预测值
    preds_matrix = np.array(list(current_predictions_test.values())).T  # shape: (num_rows, num_columns)

    # 平均置信度（按行）
    row_confidences = np.mean(preds_matrix, axis=1)

    # 找出置信度最低的5条
    lowest_conf_indices = np.argsort(row_confidences)[:num_samples]
    print("置信度最低的样本索引: ", lowest_conf_indices)

    low_conf_samples_dirty = dataSet.dirty_pd.iloc[lowest_conf_indices]
    low_conf_samples_clean = dataSet.clean_pd.iloc[lowest_conf_indices]
    print("=== 打印低置信度样本（供LLM使用） ===")
    print(low_conf_samples_dirty)
    print(low_conf_samples_clean)

    tmp_dir = "./tmp_llm_gen/"
    if not os.path.exists(tmp_dir):
        os.makedirs(tmp_dir)
    dirty_path = os.path.join(tmp_dir, "low_conf_dirty.csv")
    clean_path = os.path.join(tmp_dir, "low_conf_clean.csv")
    low_conf_samples_dirty.to_csv(dirty_path, index=False)
    low_conf_samples_clean.to_csv(clean_path, index=False)
    llm_gen_dir = "/home/stu/pys/CLLM-main/data/error_detection/hospital/enhanced_data"
    target_dir = "/home/stu/pys/ExampleDrivenErrorDetection-master/datasets/HOSP_HoloClean/enhanced_data"
    llm_dirty_path = os.path.join(llm_gen_dir, "enhanced_dirty_data_hospital_1000.csv")
    llm_clean_path = os.path.join(llm_gen_dir, "enhanced_clean_data_hospital_1000.csv")
    target_dirty_path = os.path.join(target_dir, "enhanced_dirty_data_hospital_1000.csv")
    target_clean_path = os.path.join(target_dir, "enhanced_clean_data_hospital_1000.csv")
    llm_script_path = "/home/stu/pys/CLLM-main/src/error_detection/llm_aug.py"
    # 这里是 LLM 调用的占位逻辑，你可以后续补充具体的 API 调用或生成逻辑
    subprocess.call([
        "conda", "run", "-n", "cllm_env", "python",
        llm_script_path,
    ...
    shutil.copy(llm_dirty_path, target_dirty_path)
    shutil.copy(llm_clean_path, target_clean_path)
    generated_dirty = pd.read_csv(target_dirty_path, header=0, dtype=object)
    generated_clean = pd.read_csv(target_clean_path, header=0, dtype=object)
    


    # 原始行数
    old_size = dataSet.dirty_pd.shape[0]

    # 拼接数据
    dataSet.dirty_pd = pd.concat([dataSet.dirty_pd, generated_dirty], axis=0, ignore_index=True)
    dataSet.clean_pd = pd.concat([dataSet.clean_pd, generated_clean], axis=0, ignore_index=True)

    # 更新索引（新增和之前的训练样本作为训练样本）
    new_indices = np.arange(old_size, dataSet.dirty_pd.shape[0])
    dataSet.train_indices = np.concatenate([dataSet.train_indices, new_indices])


    print("已添加 {} 条新样本，现在共有 {} 条。".format(len(new_indices), dataSet.dirty_pd.shape[0]))
...
def create_features(dataSet, train_indices, test_indices, ngrams=2, is_word=False, use_tf_idf=True):
    feature_name_list = []
    feature_list_train = []
    feature_list_test = []
    ...
    for column_id in range(dataSet.shape[1]):
        ...
        try:
            ...
            print("Warning: Column {} failed to process: {}".format(column_id, str(e)))
            pass
