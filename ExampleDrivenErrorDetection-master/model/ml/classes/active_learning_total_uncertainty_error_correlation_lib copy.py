...
def run(dataSet,
	...
	for check_this in range(checkN):
		...
		train_indices, test_indices = dataSet.train_indices, dataSet.test_indices
		...
		for round in range(label_iterations * dataSet.shape[1]):
			...
			if (round % (dataSet.shape[1]-1)) == 0 and round > 0:
				print("update train data")
				start_time_create = time.time()
				dataSet = create_train_data(dataSet, current_predictions_test)
				print("create train data: %s seconds ---" % (time.time() - start_time_create))
